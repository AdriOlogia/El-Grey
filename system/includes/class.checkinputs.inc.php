...
class cCheckInput {
	...
	private static $rxdirpos = "/^[0-9A-Za-zƒŠŒŽšœžŸÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿ_\-\s\.\(\),']*$/i";
...
*/
	...
	static function DNI($string, $index = null, $label = null) {
		...
		if ((strlen($string) == 0) and (self::$checkempty)) {
			...
			if (strlen($string) > 0) {
				if ((strlen($string) < 7) or (strlen($string) > 8)) {
					self::putMsgErr('dni',1, $index, $label);
				} else {
	...
	static function ValidarCUIT($string) {
		$result = false;
...
/* Esto tanto sirve para validar nombres de usuarios como contraseñas */
		...
		if ((strlen($string) == 0) and (self::checkempty)) {
			...
			if (strlen($string) > 0) {
				if (strlen($string) == 4) {
					...
					if (strlen($string) == 8) {
						if (!preg_match($this->rxcpa,$string)) {
	...
	private static function putMsgErr($msgindex, $shift, $index, $label) {
